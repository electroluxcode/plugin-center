import { PluginMetadata, PluginTypeValue, PluginType } from "./types"
...
export function getPluginExportsFromContent(
	...
	try {
		...
		// 注意：先处理 export { } 语句，避免匹配到 async export() 这样的方法名
		const transformedCode = content
			// 处理 export { a, b } -> __pluginExports.a = a; __pluginExports.b = b;
			// 支持多行 export 语句，使用非贪婪匹配避免匹配到嵌套的大括号
			.replace(/export\s*\{([\s\S]*?)\}/g, (_match, exports) => {
				// 分割导出项（支持逗号、换行、制表符分隔）
				const names = exports
					.split(/[,\n\r\t]+/)
					.map((n: string) => n.trim())
			...
			.replace(/export\s+default\s+([^;]+);?/g, (_match, value) => {
			...
			// 注意：避免匹配 async export() 这样的方法名
			.replace(
				/export\s+(const|let|var|function|class|async\s+function)\s+(\w+)([^;]*);?/g,
				(match, keyword, name, rest) => {
					// 如果匹配到的是 async export() 这样的方法，跳过
					if (name === "export" && rest.includes("(")) {
						return match
					}
